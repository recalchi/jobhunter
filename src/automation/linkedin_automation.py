    ...
    def search_jobs(self, job_types, location="São Paulo", salary_min=1900):
        ...
        try:
            for job_type in job_types:
                ...
                search_field = self.wait_for_element(By.XPATH, "//input[contains(@id, 'jobs-search-box-keyword-id') or contains(@aria-label, 'Search by title') or contains(@placeholder, 'Pesquisar')]")
                if search_field:
                    search_field.clear()
                ...
                location_field = self.wait_for_element(By.XPATH, "//input[contains(@id, 'jobs-search-box-location-id') or contains(@aria-label, 'Search by location') or contains(@placeholder, 'Localização')]")
                if location_field:
                ...
                search_button = self.wait_for_element(By.XPATH, "//button[contains(@class, 'jobs-search-box__submit-button') or @type='submit' or contains(@aria-label, 'Pesquisar')]")
                if search_button:
                    search_button.click()
                    self.safe_sleep(3)
                else:
                ...
                for selector in filter_selectors:
                    ...
                    if filter_button:
                        try:
                            ...
                            self.logger.warning(f"Erro ao clicar no filtro: {e}")
                            continue
                ...
                for selector in easy_apply_selectors:
                    ...
                    if easy_apply_element:
                        try:
                            ...
                            self.logger.warning(f"Erro ao ativar candidatura simplificada: {e}")
                            continue
                ...
                for selector in show_results_selectors:
                    ...
                    if show_button:
                        try:
                            ...
                            self.logger.warning(f"Erro ao exibir resultados: {e}")
                            continue
